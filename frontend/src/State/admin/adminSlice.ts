...
const initialState: HomeCategorySate = {
  categories: [],
  loading: false,
  error: null,
...
const adminSlice = createSlice({
    ...
    extraReducers:(builder)=>{
        ...
        // Handle the filfilled state for updateHomeCategory
        builder.addCase(updateHomeCategory.fulfilled, (state, action)=>{
            ...
            if (index !== -1) {
                ...
                state.categories.push(action.payload)  //If the category doesn't exist , add item 
            }
        });
